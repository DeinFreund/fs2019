...
void copyToDevice(gridLevel * g, size_t s, size_t gridCount) {
  ...
  for (size_t l = s; l<gridCount; l++) {
    cudaMemcpy(g[l].gU, g[l].U, sizeof(double) * g[l].N * g[l].N, cudaMemcpyHostToDevice);
    checkCUDAError("Error copying U");
    cudaMemcpy(g[l].gRes, g[l].Res, sizeof(double) * g[l].N * g[l].N, cudaMemcpyHostToDevice);
    checkCUDAError("Error copying Res");
    cudaMemcpy(g[l].gUn, g[l].Un, sizeof(double) * g[l].N * g[l].N, cudaMemcpyHostToDevice);
    checkCUDAError("Error copying Un");
    cudaMemcpy(g[l].gf, g[l].f, sizeof(double) * g[l].N * g[l].N, cudaMemcpyHostToDevice);
    checkCUDAError("Error copying f");
...
void copyToHost(gridLevel * g, size_t s, size_t gridCount) {
  ...
  for (size_t l = s; l<gridCount; l++) {
    cudaMemcpy(g[l].U, g[l].gU, sizeof(double) * g[l].N * g[l].N, cudaMemcpyDeviceToHost);
    checkCUDAError("Error copying U back");
    cudaMemcpy(g[l].Res, g[l].gRes, sizeof(double) * g[l].N * g[l].N, cudaMemcpyDeviceToHost);
    checkCUDAError("Error copying Res back");
    cudaMemcpy(g[l].Un, g[l].gUn, sizeof(double) * g[l].N * g[l].N, cudaMemcpyDeviceToHost);
    checkCUDAError("Error copying Un back");
    cudaMemcpy(g[l].f, g[l].gf, sizeof(double) * g[l].N * g[l].N, cudaMemcpyDeviceToHost);
    checkCUDAError("Error copying f back");
    cudaDeviceSynchronize();
...
void runReduction(int dimGrid, int dimBlock, int smemSize, double * d_idata, double * d_odata) {
  switch (dimBlock) {
  case 2048:
    reduce5<2048><<<dimGrid, dimBlock, smemSize>>>(d_idata, d_odata);
    break;
  case 1024:
    reduce5<1024><<<dimGrid, dimBlock, smemSize>>>(d_idata, d_odata);
    break;
  case 512:
...
void calculateL2Norm(gridLevel * g, size_t l) {
  ...
  cudaMemcpy(g[l].ResSum, g[l].gResSum, sizeof(double) * blocks, cudaMemcpyDeviceToHost);
  checkCUDAError("Error copying sum results back");
  cudaDeviceSynchronize();
...
void restrict(double * Res, double * f, size_t N, size_t Nold) {
  ...
  size_t i = k / Nold;
  size_t j = k % Nold;
  if (i == 0 || i>= Nold - 1 || j == 0 || j>= Nold - 1) return;
...
void prolong(double * U, double * Uold, size_t N, size_t Nold) {
  ...
  size_t i = k / Nold;
  size_t j = k % Nold;
  if (!(i == 0 || i>= Nold - 1 || j == 0 || j>= Nold - 1))
    U[2 * i * N + 2 * j] += Uold[i * Nold + j];

  if (!(i == 0 || i>= Nold || j == 0 || j>= Nold - 1))
    U[(2 * i - 1) * N + 2 * j] += (Uold[(i - 1) * Nold + j] + Uold[i * Nold + j]) * 0.5;

  if (!(i == 0 || i>= Nold - 1 || j == 0 || j>= Nold))
    U[(2 * i) * N + 2 * j - 1] += (Uold[i * Nold + j - 1] + Uold[i * Nold + j]) * 0.5;

  if (!(i == 0 || i>= Nold || j == 0 || j>= Nold))
...
gridLevel * generateInitialConditions(size_t N0, size_t gridCount) {
  ...
  for (size_t i = 0; i<gridCount; i++) {
    ...
    cudaMalloc( & g[i].gU, sizeof(double) * g[i].N * g[i].N);
    checkCUDAError("Error allocating gU");
    cudaMalloc( & g[i].gUn, sizeof(double) * g[i].N * g[i].N);
    checkCUDAError("Error allocating gUn");
    cudaMalloc( & g[i].gRes, sizeof(double) * g[i].N * g[i].N);
    checkCUDAError("Error allocating gRes");
    cudaMalloc( & g[i].gf, sizeof(double) * g[i].N * g[i].N);
    checkCUDAError("Error allocating gf");
    cudaMalloc( & g[i].gResSum, sizeof(double) * std::max(g[i].blocksPerGrid, 100));
    checkCUDAError("Error allocating gResSum");
...
void printTimings(size_t gridCount) {
  ...
  printf("   Time (s)    ");
  for (size_t i = 0; i<gridCount; i++) printf("Grid%lu   ", i);
  printf("   Total  \n");
  printf("-------------|-");
  for (size_t i = 0; i<gridCount; i++) printf("--------");
  printf("|---------\n");
  printf("Smoothing    | ");
  for (size_t i = 0; i<gridCount; i++) printf("%2.3f   ", smoothingTime[i]);
  printf("|  %2.3f  \n", totalSmoothingTime);
  printf("Residual     | ");
  for (size_t i = 0; i<gridCount; i++) printf("%2.3f   ", residualTime[i]);
  printf("|  %2.3f  \n", totalResidualTime);
  printf("Restriction  | ");
  for (size_t i = 0; i<gridCount; i++) printf("%2.3f   ", restrictionTime[i]);
  printf("|  %2.3f  \n", totalRestrictionTime);
  printf("Prolongation | ");
  for (size_t i = 0; i<gridCount; i++) printf("%2.3f   ", prolongTime[i]);
  printf("|  %2.3f  \n", totalProlongTime);
  printf("L2Norm       | ");
  for (size_t i = 0; i<gridCount; i++) printf("%2.3f   ", L2NormTime[i]);
  printf("|  %2.3f  \n", totalL2NormTime);
  printf("-------------|-");
  for (size_t i = 0; i<gridCount; i++) printf("--------");
  printf("|---------\n");
  printf("Total        | ");
  for (size_t i = 0; i<gridCount; i++) printf("%2.3f   ", timePerGrid[i]);
  printf("|  %2.3f  \n", totalMeasured);
  printf("-------------|-");
  for (size_t i = 0; i<gridCount; i++) printf("--------");
  printf("|---------\n");
